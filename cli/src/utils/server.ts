...
// Resolve the fnox config path, falling back to legacy locations for servers
// that haven't run the bootstrap migration yet. Keep this in sync with
// `getFnoxConfigPath()` in `server/lib/settings/fnox.ts`.
function resolveFnoxConfigPath(fulcrumDir: string): string {
 ...
 * 4. FULCRUM_DIR config/fnox.toml (read port)
 * 5. .fulcrum/config/fnox.toml in CWD (read port)
 * 6. ~/.fulcrum/config/fnox.toml (read port)
...
export function discoverServerUrl(urlOverride?: string, portOverride?: string): string {
  ...
  // 6. Global ~/.fulcrum/config/fnox.toml
